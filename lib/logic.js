...
/**
 * Sign a departure and confirmation.
 * @param {org.blockowisko.hypercmrsystem.DriverDepartureSignature} driverDepartureSignature
 * @transaction
 */
async function driverDepartureSignature(tx) {
    const documentID = tx.documentID;
    const notes = tx.notes;
    const status = 45;
    const statusMessage = 'W drodze';

    const assetRegistry = await getAssetRegistry('org.blockowisko.hypercmrsystem.CMRDocument');
    const document = await assetRegistry.get(documentID);

    document.status = status;
    document.statusMessage = statusMessage;
    document.driverNotes = notes;
